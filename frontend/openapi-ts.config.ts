...
// committed source artifact. We regenerate the client from it via `npm run gen:api`.
  ...
  plugins: [
    {
      ...
      // baseUrl + auth live in a hand-written wrapper so the generated client
      // stays a pure artifact (see shared/api/runtime.ts).
