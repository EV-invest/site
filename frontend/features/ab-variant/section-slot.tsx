...
export function SectionSlot({ section }: { section: SectionInstance }) {
  // Start empty so SSR and the first client render agree (no `window` on the
  // server); read the real hash once mounted.
  ...
  const Component =
    variant === "b" ? section.b() : variant === "c" ? section.c() : section.a();
  ...
  // Production renders the bare slot; the dev-only A/B cycling wrapper is
  // dead-code-eliminated from the production bundle.
  if (process.env.NODE_ENV === "production") return rendered;
  return (
    <DevVariantToggle section={section} variant={variant}>
      {rendered}
    </DevVariantToggle>
  );
}
...
function DevVariantToggle({
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      window.location.hash =
        next === "a" ? section.name : `${section.name}.${next}`;
    };
  ...
  return (
    <div
      ...
      className={
        focused
          ? "outline-dashed outline-1 outline-main-accent-t1/40"
          : undefined
      }
    >
      {children}
