...
fn main() -> ui_test::color_eyre::Result<()> {
	...
	config.program.args.push(format!("ev_site_macros={}", proc_macro.display()).into());
...
fn find_proc_macro() -> PathBuf {
	...
	let mut candidates: Vec<_> = std::fs::read_dir(&deps_dir)
		...
		.filter(|p| {
			...
			name.starts_with("libev_site_macros") && (name.ends_with(".so") || name.ends_with(".dylib") || name.ends_with(".dll"))
		})
		.collect();

	candidates.sort_by_key(|p| std::fs::metadata(p).and_then(|m| m.modified()).ok());
	candidates.pop().unwrap_or_else(|| panic!("compiled ev_site_macros not found in {}", deps_dir.display()))
}
